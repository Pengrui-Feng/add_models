    ...
    for j in range(n*m,n*m+m):
        for i in range(len(t_ids)):       
            if obsID[j]==t_ids[i]:  # to give the different color line for each turtle
                ...
                ax.text(obsTemp[j][-1]-6,obsDepth[j][-1]+3,round((obsTemp[j][-1]*1.8+30),1),color='r',fontsize=10)                
                ax.text(dopTemp[j][-1]+1,obsDepth[j][-1]+3,round((dopTemp[j][-1]*1.8+30),1),color='g',fontsize=10)                              
                
                ax.text(obsTemp[j][0]-8,obsDepth[j][0]-1,round((obsTemp[j][0]*1.8+30),1),color='r',fontsize=10)
                ax.text(dopTemp[j][0]-2,obsDepth[j][0]-1,round((dopTemp[j][0]*1.8+30),1),color='g',fontsize=10)

    time=obsTime[n].strftime('%m-%d-%Y')           
    ax.set_ylim([maxdepth,-10])
    ax.set_xticks([int(obsTemp[n][-1]-6), int(dopTemp[n][0])+5])
    ax.text(15, 195,'%s'%time,ha='center', va='center',color='black',fontsize=10)
