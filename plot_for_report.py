...
data1=data.groupby(['PTT','gps_date'])['depth','obs_temp','doppio_temp','FVCOM_temp'].agg(lambda x:x.str.cat(sep=','))
...
Time = pd.Series((datetime.strptime(x, '%Y-%m-%d %H:%M:%S') for x in obsData['gps_date']))
...
Data.index=range(8)
obsTime =  pd.Series((datetime.strptime(x, '%Y-%m-%d %H:%M:%S') for x in Data['gps_date']))
    ...
    for j in range(n*m,n*m+m):
        for i in range(len(t_ids)):       
            if obsID[j]==t_ids[i]:  # to give the different color line for each turtle
                ...
                ax.text(obsTemp[j][-1]-6,obsDepth[j][-1]+3,int(obsTemp[j][-1]),color='r',fontsize=10)                
                ax.text(dopTemp[j][-1]+1,obsDepth[j][-1]+10,int(dopTemp[j][-1]),color='g',fontsize=10)                              
                ax.text(fvcTemp[j][-1]+1,obsDepth[j][-1]+3,int(fvcTemp[j][-1]),color='b',fontsize=10)
                
                ax.text(obsTemp[j][0]-3,obsDepth[j][0]-4,int(obsTemp[j][0]),color='r',fontsize=10)
                ax.text(dopTemp[j][0]+1,obsDepth[j][0]-4,int(dopTemp[j][0]),color='g',fontsize=10)
                ax.text(fvcTemp[j][0]-9,obsDepth[j][0]-4,int(fvcTemp[j][0]),color='b',fontsize=10)
    time=obsTime[n].strftime('%m-%d-%Y')           
    ax.set_ylim([maxdepth+10,-10])
    ax.set_xticks([int(obsTemp[n][-1]-6), int(dopTemp[n][0])+8])
    ax.text(57, 205,'%s'%time,ha='center', va='center',color='black',fontsize=10)
